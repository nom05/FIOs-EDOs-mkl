...
contains
  ...
  subroutine quit_program_warning(message, status,nocolor)
    ...
    if (.NOT.nocolordef) then
       write(*,'(//,A)')'  * '//achar(27)//'[33mWarning:'//achar(27)//'[0m  '//&
                                trim(program_name)//':  '//trim(message)//' *'
    else
       write(*,'(//,A)')'  * Warning:'// &
                                trim(program_name)//':  '//trim(message)//' *'
    endif !! (.NOT.nocolordef) then
  ...
  subroutine quit_program_error(message, status,nocolor)
    use SUFR_constants, only: program_name
    implicit none
    character, intent(in)         :: message*(*)
    integer, intent(in)           :: status
    ...
    if (present(nocolor)) nocolordef = nocolor
    if (.NOT.nocolordef) then
       write(0,'(//,A)')'  ***  '//achar(27)//'[31mERROR:'//achar(27)//'[0m '//&
                                trim(program_name)//':  '//trim(message)//'  ***'
    else
       write(0,'(//,A)')'  ***  ERROR: '//&
                                trim(program_name)//':  '//trim(message)//'  ***'
  ...
  subroutine warn(message, unit, nocolor)
    use SUFR_constants, only: program_name
    implicit none
    character, intent(in) :: message*(*)
    ...
    if (.NOT.nocolordef) then
       write(u,'(/,A,/)')'  * '//achar(27)//'[33mWarning:'//achar(27)//'[0m  '//&
                                trim(program_name)//':  '//trim(message)//' *'
    else
